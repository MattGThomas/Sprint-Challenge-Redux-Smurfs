...
- [ ] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
  ...
  actions send information to the store
  reducers describe how app will change based on the actions being sent
  store
  the store brings the actions and reducers together, the store allows us to have an initial state for the application that can be passed down to any children components, while also being updated and automatically passing the updates down

- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?

application state is global, meaning any component anywhere can access it, component state can only be updated within that specific component. bigger applications are more likely to use application state.

- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

gives ability to wait for promises to resovle before action is sent to reducer, allows action creators to return a function instead of an action
